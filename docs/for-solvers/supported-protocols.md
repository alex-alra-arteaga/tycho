...
<tr><td><code>flamm</code></td><td>Native (<code>FlammPoolState</code>)</td><td>-</td><td>Base</td><td>Everlong FLAMM: two components per pool, the swap venue (id = the pool address, both directions) and the lever-up venue (id = <code>pool || 0x00000000 || uint64(1)</code>, pool asset in only; lever-down is not quoted). Quotes are exact at the execution block (<code>apply_block</code>, which re-emits a positioned pool on every block and an unpositioned one only across a feed, sequencer, spread, oracle-reveal, scheduled-change or rate-ceiling deadline) and only for sizes the pool fills in full: <code>get_limits</code> is the trait's soft limit, a size that fills in full rather than a threshold (a buy above it can fill too, the price band being periodic in the size), a size the pool would clip is refused, and a buy of dust (a few thousand USDC units, an absolute bound rather than a fraction of the limit) is the empty trade. <code>spot_price(base, quote)</code> follows the trait (the <code>quote</code> that buys one <code>base</code>, gross of that direction's fee); the lever-up venue has one rate and answers both orderings from it. Register with <code>flamm_filter</code>: a component whose snapshot the decoder refuses (unmodelled code, wiring drift, an unsupported configuration) is then skipped instead of failing the stream. Execution: <code>FLAMMExecutor</code>, pending deployment and whitelisting — no executor address is registered for <code>flamm</code> on Base yet, so the default encoder registry has no <code>flamm</code> encoder.</td></tr>
    ...
    match chain {
        ...
        Chain::Base => {
            builder = builder
                ...
                .exchange::<FlammPoolState>("flamm", tvl_filter.clone(), Some(flamm_filter))
        }
        Chain::Unichain => {
            builder = builder
                .exchange::<UniswapV2State>("uniswap_v2", tvl_filter.clone(), None)
